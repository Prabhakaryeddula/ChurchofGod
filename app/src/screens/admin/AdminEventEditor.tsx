...
// Event type is now a free-text field — no picklist restriction
...
export default function AdminEventEditor() {
  ...
  const handleSave = async (status: 'Published' | 'Draft') => {
    ...
    // 🌍 Auto-geocode: silently derive Lat/Lng from Venue + Address + PIN Code
    let geoCoords: { lat: number; lng: number } | null = null;
    try {
      const GOOGLE_KEY = process.env.EXPO_PUBLIC_GOOGLE_MAPS_KEY || 'AIzaSyCfUn5Hktkz58RBl3BRa5jXd_71uraI-cA';
      const fullAddr = [venueEn, address, pinCode].filter(Boolean).join(', ');
      if (fullAddr) {
        const geoResp = await fetch(
          `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(fullAddr)}&key=${GOOGLE_KEY}`
        );
        const geoData = await geoResp.json();
        if (geoData.status === 'OK' && geoData.results?.length > 0) {
          const loc = geoData.results[0].geometry.location;
          geoCoords = { lat: loc.lat, lng: loc.lng };
          console.log('📍 [AdminEventEditor] Auto-geocoded:', geoCoords);
        } else {
          console.warn('⚠️ [AdminEventEditor] Geocoding returned no results for:', fullAddr);
        }
      }
    } catch (geoErr) {
      console.warn('⚠️ [AdminEventEditor] Geocoding failed (non-fatal):', geoErr);
    }
    ...
    const payload = {
      ...
      descEn, descTe, venueEn, venueTe, address, pinCode,
  ...
  const resetForm = () => {
    ...
    setVenueEn(''); setVenueTe(''); setAddress(''); setPinCode('');
    setBannerUrl('');
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.inputGroup}>
          ...
          <TextInput
            style={styles.input}
            placeholder="e.g. Sunday Service, Prayer Meeting, Conference..."
            value={eventType}
        ...
        <View style={styles.inputGroup}>
          <Text style={styles.label}>
            PIN Code{'  '}<Text style={styles.labelSub}>Improves Google Maps accuracy</Text>
          </Text>
          <TextInput
            style={styles.input}
        ...
        <View style={[styles.inputGroup, styles.geocodeBadge]}>
          <MapPin size={12} color="#15803D" />
          <Text style={styles.geocodeBadgeText}>
            Latitude &amp; Longitude are auto-computed from the address above — no manual entry needed.
          </Text>
        </View>
          ...
          <View style={[styles.cardPreview, { backgroundColor: bannerColor }]}>
            ...
            <View style={styles.cardOverlay}>
              <View style={styles.cardTypeRow}>
                <Text style={styles.cardType}>● {eventType || 'Event Type'}</Text>
...
const styles = StyleSheet.create({
  ...
  geocodeBadge: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#f0fdf4', borderRadius: 8, paddingVertical: 8, paddingHorizontal: 12, borderWidth: 1, borderColor: '#bbf7d0', marginBottom: 4 },
  geocodeBadgeText: { fontSize: 11, color: '#15803D', fontWeight: '500', flex: 1, lineHeight: 16 },
