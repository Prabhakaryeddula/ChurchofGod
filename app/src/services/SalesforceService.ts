...
-----END PRIVATE KEY-----`;
  ...
  async fetchEvents(limit = 10): Promise<any[]> {
    try {
      ...
      const parseDT = (dt: string) => {
        ...
        hour = hour % 12 || 12;
        return { date: datePart, time: `${String(hour).padStart(2, '0')}:${m} ${ampm}` };
      };

      return result.records.map((rec: any) => {
        const start = parseDT(rec.StartDateTime);
        const end   = parseDT(rec.EndDateTime);
        return {
          id: rec.Id,
          name: rec.Subject,
  ...
  async getEventMetadata(): Promise<any> {
    try {
      const token = await this.getAccessToken();
      const resp = await fetch(`${this.instanceUrl}/services/data/v60.0/sobjects/Event/describe`, {
    ...
    try {
      ...
      return result.records.map((rec: any) => {
        ...
        return {
          ...
          startTime: start.time,
          endTime: end.time,
          descEn: rec.Description,
          descTe: rec.Description_Telugu__c,
          venueEn: rec.Location,
  ...
  async createEvent(details: any) {
    try {
      ...
      const url = `${this.instanceUrl}/services/data/v60.0/sobjects/Event${isUpdate ? '/' + details.id : ''}`;

      // Combine date (YYYY-MM-DD) + time (HH:mm:00.000Z) → ISO StartDateTime / EndDateTime
      const toISO = (date: string, sfTime: string | null) => {
        if (!date || !sfTime) return undefined;
        return `${date}T${sfTime}`;
      };
      ...
      const body: any = {
        ...
        Location: details.venueEn,
        Location_Telugu__c: details.venueTe,
        // Store full address + PIN Code together in Address__c
        Address__c: details.pinCode
          ? `${details.address}, ${details.pinCode}`.trim()
          : details.address,
